...
This vignette illustrates how to perform allele specific copy number inference with DLP+ data. The method should be applicable to any single cell copy number method however.

## Data
The data needed to perform the allele specific copy number inference are the binned copy number data and cell specific haplotype calls. The binned data and the haplotype data should be binned in the same manner.
...
The haplotype data comes in long format, but the package requires data in wide format so we will use the helper function `r format_haplotypes_dlp` to convert the data to the correct format. 
```{r}
haplotypes <- format_haplotypes_dlp(haplotypes)
head(haplotypes)
```

## Allele specific inference

The output of `schnapps` is allele specific states per bin. `schnapps` has 2 options to perform this inference, the first `callAlleleSpecificCN`, infers allele specific states using mirrored B allele frequencies as is typically done in bulk whole genome sequencing. This infers states of the form `A|B` where `B < A`. The second option, `callHaplotypeSpecificCN` takes into account the possibility that different alleles may be lost or gained in different cells. This method phases alleles jointly across cells so that these events can be identified.
