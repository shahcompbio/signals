...
```{r}
CNBAF <- combineBAFCN(haplotypes, CNbins, binsize = 5e6, filtern = 0)
...
alleleCN %>%
    ...
    summarise(state = schaplotypeR:::Mode(state), 
              state_min = schaplotypeR:::Mode(state_min), 
              BAF = median(BAF), 
              state_phase = schaplotypeR:::Mode(state_phase), 
              copy = median(copy)) %>%
    ungroup() %>%
