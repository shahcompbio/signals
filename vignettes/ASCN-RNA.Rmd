...
ascn_rna_dp <- assign_states_dp(haplotypes_per_segment$bafpersegment)
ascn_rna_dp
```
...
```{r}
x <- createBAFassay(x, ascn_rna_dp, haplotypes_rna)
```
...
```{r}
rownames(x@assays$BAF)
FeaturePlot(x, "BAF-3-75500001-178500000") + scale_color_gradientn(colors = scBAFstate_cols())
```

```{r}
FeaturePlot(x, "BAF-10-500001-97000000") + scale_color_gradientn(colors = scBAFstate_cols())
```
