...
fix_assignments2 <- function(hscn) {
  if (hscn$likelihood$likelihood == "binomial") {
    suppressWarnings(hscn_data <- hscn$data %>%
      ...
      .[, Minafter := shift(Min, fill = 0, type = "lead")] %>%
      .[, state_min := fifelse(
        nbins == 1 & state == stateprev,
        Minprev,
        state_min
      )] %>%
      .[, state_min := fifelse(
        nbins == 1 & state == stateafter,
        Minafter,
        state_min
      )] %>%
      .[, Maj := state - state_min] %>%
    ...
    suppressWarnings(hscn_data <- hscn$data %>%
      ...
      add_states() %>%
      dplyr::select(
        -stateafter, -stateprev,
        -Minafter, -Minprev, -rlid
      ))
    ...
    hscn_data <- hscn_data %>%
      ...
      add_states() %>%
      dplyr::select(
        -LLassigned, -LLother, -alleleBtot,
        -alleleAtot, -pMin, -pMaj, -rlid
      )
  }
...
rephasebins <- function(cn, chromosomes = NULL, method = "mindist", whole_chr_cutoff = 0.9, ncells = 1) {
  ...
  if (method == "mindist") {
    ...
    newhscn <- as.data.table(cndat)[as.data.table(phase_df),
      on = c("chr", "start", "end")
    ] %>%
      .[, Min := ifelse(phasing == "switch", Maj, Min)] %>%
      .[, Maj := ifelse(phasing == "switch", state - Min, Maj)] %>%
